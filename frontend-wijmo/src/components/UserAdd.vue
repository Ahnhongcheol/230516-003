<template>
    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-title v-if="value._links">
            UserAdd # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
        </v-card-title >
        <v-card-title v-else>
            UserAdd
        </v-card-title >
        ...
        <v-card-text>
            <String label="TenantId" v-model="value.tenantId" :editMode="editMode"/>
            <String label="UseStatus" v-model="value.useStatus" :editMode="editMode"/>
            <String label="UserId" v-model="value.userId" :editMode="editMode"/>
            <String label="Pass" v-model="value.pass" :editMode="editMode"/>
...
import UserAddBase from '../components/UserAddBase.vue'
...
export default {
    ...
    created(){
        ...
        websocketUrl.pathname = "/rsocket/userAdds";
        websocketUrl.hash = "";
        ...
        client.connect().subscribe({
            ...
            const endpoint = "userAdds."+ me.value.id +".get"
    ...
    methods: {
        ...
        async save(){
            try {
                ...
                if(!this.offline) {
                    if(this.isNew) {
                        temp = await axios.post(axios.fixUrl('/userAdds'), this.value)
                    } else {
